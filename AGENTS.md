...
## 12. コーディングスタイル補足
- DTO・サービスなどファイル名と同名のエクスポートを最上部に定義し、その下に依存するクラスや型を呼び出し順に配置する。今後ファイルを編集する際もこのルールを継続すること。
- 複数 DTO を同一ファイルで定義する場合は、主役 DTO → 参照されるサブ DTO → ヘルパー型の順で並べ、Swagger 用デコレーターでは `type: () => SubDto` のように遅延評価を利用して循環を避ける。
