...
- **説明**: ランダム、またはクエリで指定したサーヴァントIDのスキル情報を含むクイズデータを返す
- **クエリパラメータ**:
  - `servantId` (任意): 指定すると該当サーヴァントのスキルクイズを取得
- **レスポンス**: `ServantDetailGetResponseDto`

### 3. プロフィールクイズ取得

- **エンドポイント**: `GET /quiz/profile`
- **説明**: ランダム、またはクエリで指定したサーヴァントIDのプロフィール情報を含むクイズデータを返す
- **クエリパラメータ**:
  - `servantId` (任意): 指定すると該当サーヴァントのプロフィールクイズを取得
- **レスポンス**: `ServantProfileGetResponseDto`

### 4. 宝具クイズ取得

- **エンドポイント**: `GET /quiz/np`
- **説明**: ランダム、またはクエリで指定したサーヴァントIDの宝具情報を含むクイズデータを返す
- **クエリパラメータ**:
  - `servantId` (任意): 指定すると該当サーヴァントの宝具クイズを取得
- **レスポンス**: `ServantNpGetResponseDto`
